...
import { b as atom } from "./createQueryStore.js";
...
const listenInputFromContext = (config2) => ({
  context: context2
}) => {
  const { count: count2, include, exclude, responseType = "message.received" } = config2;
  return {
    count: count2,
    domain: context2.domain,
    from: context2.connectTo,
    include: include ? Array.isArray(include) ? include : [include] : [],
    exclude: exclude ? Array.isArray(exclude) ? exclude : [exclude] : [],
    responseType,
    target: context2.target,
    to: context2.name
  };
  ...
  type,
  message: event
}), messageEvents$ = cjs.defer(
  () => cjs.fromEvent(window, "message")
), createListenLogic = (compatMap) => fromEventObservable(({ input }) => messageEvents$.pipe(
  compatMap ? cjs.map(compatMap) : cjs.pipe(),
  cjs.filter(listenFilter(input)),
  cjs.map(eventToMessage(input.responseType)),
  input.count ? cjs.pipe(
    cjs.bufferCount(input.count),
    cjs.concatMap((arr) => arr),
    cjs.take(input.count)
  ) : cjs.pipe()
  ...
  actors: {
    listen: fromEventObservable(
      ({
        ...
        const abortSignal$ = input.signal ? cjs.fromEvent(input.signal, "abort").pipe(
          throwOnEvent(`Request ${input.requestId} aborted`)
        ) : cjs.EMPTY, messageFilter = (event) => event.data?.type === MSG_RESPONSE && event.data?.responseTo === input.requestId && !!event.source && input.sources.has(event.source);
        return cjs.fromEvent(window, "message").pipe(
          cjs.filter(messageFilter),
          cjs.take(input.sources.size),
          cjs.takeUntil(abortSignal$)
        );
      }
    )
  },
  actions: {
    "send message": ({ context: context2 }, params) => {
      const { sources, targetOrigin } = context2, { message } = params;
      sources.forEach((source) => {
        source.postMessage(message, { targetOrigin });
      });
    },
    "on success": sendTo(
      ({ context: context2 }) => context2.parentRef,
      ({ context: context2, self: self2 }) => (context2.response && context2.resolvable?.resolve(context2.response), {
        type: "request.success",
        requestId: self2.id,
        response: context2.response,
        responseTo: context2.responseTo
      })
    ),
    "on fail": sendTo(
      ({ context: context2 }) => context2.parentRef,
      ({ context: context2, self: self2 }) => (context2.suppressWarnings || console.warn(
        `[@sanity/comlink] Received no response to message '${context2.type}' on client '${context2.from}' (ID: '${context2.id}').`
      ), context2.resolvable?.reject(new Error("No response received")), { type: "request.failed", requestId: self2.id })
    ),
    "on abort": sendTo(
      ({ context: context2 }) => context2.parentRef,
      ({ context: context2, self: self2 }) => (context2.resolvable?.reject(new Error("Request aborted")), { type: "request.aborted", requestId: self2.id })
    )
  },
  guards: {
    expectsResponse: ({ context: context2 }) => context2.expectResponse
  },
  delays: {
    initialTimeout: 0,
    responseTimeout: ({ context: context2 }) => context2.responseTimeout ?? RESPONSE_TIMEOUT_DEFAULT
  }
}).createMachine({
    ...
    sending: {
      entry: {
        ...
        params: ({ context: context2 }) => {
          const { connectionId, data, domain, from: from2, id, responseTo, to, type } = context2;
          return { message: {
            connectionId,
            data,
            domain,
  ...
  output: ({ context: context2, self: self2 }) => ({
    requestId: self2.id,
    response: context2.response,
    responseTo: context2.responseTo
  })
  ...
  actions: {
    "buffer incoming message": assign({
      handshakeBuffer: ({ event, context: context2 }) => (assertEvent(event, "message.received"), [...context2.handshakeBuffer, event])
    }),
    "buffer message": enqueueActions(({ enqueue }) => {
      enqueue.assign({
        buffer: ({ event, context: context2 }) => (assertEvent(event, "post"), [
          ...context2.buffer,
          {
            data: event.data,
    ...
    "create request": assign({
      requests: ({ context: context2, event, self: self2, spawn }) => {
        ...
        const requests = (Array.isArray(event.data) ? event.data : [event.data]).map((request) => {
          ...
          return spawn("requestMachine", {
            id,
            input: {
              connectionId: context2.connectionId,
              data: request.data,
              domain: context2.domain,
              ...
              sources: context2.target,
              suppressWarnings: request.options?.suppressWarnings,
              targetOrigin: context2.targetOrigin,
              to: context2.connectTo,
              type: request.type
            }
          });
        });
        return [...context2.requests, ...requests];
      }
    }),
    "emit heartbeat": emit(() => ({
      type: "_heartbeat"
    ...
    "flush buffer": enqueueActions(({ enqueue }) => {
      enqueue.raise(({ context: context2 }) => ({
        type: "request",
        data: context2.buffer.map(({ data, resolvable, options }) => ({
          data: data.data,
          type: data.type,
      ...
      })), enqueue.emit(({ context: context2 }) => ({
        type: "_buffer.flushed",
        messages: context2.buffer.map(({ data }) => data)
      })), enqueue.assign({
        buffer: []
      });
    }),
    "flush handshake buffer": enqueueActions(({ context: context2, enqueue }) => {
      context2.handshakeBuffer.forEach((event) => enqueue.raise(event)), enqueue.assign({
        handshakeBuffer: []
      });
    }),
    ...
    "remove request": enqueueActions(({ context: context2, enqueue, event }) => {
      assertEvent(event, ["request.success", "request.failed", "request.aborted"]), stopChild(event.requestId), enqueue.assign({ requests: context2.requests.filter(({ id }) => id !== event.requestId) });
    }),
    ...
    "set connection config": assign({
      connectionId: ({ event }) => (assertEvent(event, "handshake.syn"), event.message.data.connectionId),
  ...
  guards: {
    hasSource: ({ context: context2 }) => context2.target !== null
  }
}).createMachine({
  ...
  const actor = createActor(machine, {
    ...
    const { responseTimeout = FETCH_TIMEOUT_DEFAULT, signal, suppressWarnings } = options || {}, resolvable = Promise.withResolvers();
    return actor.send({
      type: "post",
      data,
      resolvable,
...
function enableLiveMode(options) {
  ...
  const { projectId, dataset } = client.config(), $perspective = atom("previewDrafts"), $connected = atom(false), cache2 = /* @__PURE__ */ new Map(), comlink = createNode(
    {
      name: "loaders",
  ...
  comlink.onStatus((status) => {
    status === "connected" ? $connected.set(true) : status === "disconnected" && $connected.set(false);
    ...
    if (data.projectId === projectId && data.dataset === dataset) {
      ...
      $perspective.set(data.perspective), updateLiveQueries();
    }
  }), comlink.on("loader/query-change", (data) => {
    if (data.projectId === projectId && data.dataset === dataset) {
      const { perspective, query, params } = data;
      data.result !== void 0 && data.resultSourceMap !== void 0 && client.config().stega.enabled ? cache2.set(JSON.stringify({ perspective, query, params }), {
        ...data,
      ...
      }) : cache2.set(JSON.stringify({ perspective, query, params }), data), updateLiveQueries();
    }
  });
  let unsetFetcher;
  const unlistenConnection = $connected.listen((connected) => {
    connected ? (unsetFetcher = setFetcher({
      hydrate: (query, params, initial) => {
        const perspective = initial?.perspective || $perspective.get(), key = JSON.stringify({
          perspective,
          query,
          params
        }), snapshot = cache2.get(key);
    ...
    const interval2 = setInterval(() => emitQueryListen(true), LISTEN_HEARTBEAT_INTERVAL);
    return () => {
      clearInterval(interval2), liveQueries.delete(liveQuery), emitQueryListen();
    };
    ...
    for (const { query, params, $fetch } of liveQueries)
      ...
      }), !skipSetLoading && $connected.value === true && $fetch.setKey("loading", true), $fetch.setKey("perspective", perspective);
  };
  function updateLiveQueries() {
    const perspective = $perspective.get(), documentsOnPage = [];
    for (const { query, params, $fetch } of liveQueries) {
      const key = JSON.stringify({ perspective, query, params }), value = cache2.get(key);
      value && ($fetch.set({
        data: value.result,
        error: void 0,
        loading: false,
  ...
  return () => {
    unsetFetcher?.(), unlistenConnection(), stop();
  };
}
export {
  enableLiveMode
