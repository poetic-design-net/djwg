<script lang="ts">
  ...
  export let hasAccess: boolean = true;
  ...
  function handleEmailClick() {
    if (partner.email) {
      let mailtoUrl = `mailto:${partner.email}`;
      
      // Add subject if available
      if (partner.emailSubject) {
        mailtoUrl += `?subject=${encodeURIComponent(partner.emailSubject)}`;
      }
      
      // Add body if available
      if (partner.emailBody) {
        if (partner.emailSubject) {
          mailtoUrl += `&body=${encodeURIComponent(partner.emailBody)}`;
        } else {
          mailtoUrl += `?body=${encodeURIComponent(partner.emailBody)}`;
        }
      }
...
<div class="relative rounded-xl p-6 border border-gray-800 overflow-hidden hover:border-gray-700 transition-colors duration-200 {!hasAccess ? 'locked-partner' : ''}">
  ...
  <div class="relative">
    ...
    <div class="flex justify-between items-start gap-4 mb-4">
      ...
      <!-- Partner Badge Icon -->
      <div class="{hasAccess ? 'text-amber-400' : 'text-gray-500'}">
        {#if hasAccess}
      ...
      <div class="bg-gradient-to-r from-green-900/30 to-emerald-900/30 rounded-lg p-3 mb-4 border border-green-800/30">
        <div class="flex items-center justify-between">
          <div>
            ...
            {:else}
              <code class="text-green-300 font-mono text-lg blur-content">******</code>
            {/if}
    ...
    <div class="flex gap-2 flex-wrap">
      {#if partner.website && (hasAccess || partner.showWebsiteWithoutBadge)}
        <button 
          on:click={hasAccess ? handleWebsiteClick : null}
          class="flex items-center gap-2 px-3 py-2 bg-blue-600/20 text-blue-400 rounded-md transition-colors text-sm border border-blue-600/30 {hasAccess ? 'hover:bg-blue-600/30 cursor-pointer' : 'opacity-50 cursor-not-allowed'}"
          title={hasAccess ? "Website besuchen" : "Partner Badge erforderlich"}
          disabled={!hasAccess}
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" />
          </svg>
          <span class="{!hasAccess ? 'blur-content' : ''}">Website</span>
        </button>
      {/if}
      ...
      {#if partner.email}
        <div class="flex flex-col gap-1 ">
          <button
            on:click={hasAccess ? handleEmailClick : null}
            class="flex items-center gap-2 px-3 py-2 bg-purple-600/20 text-purple-400 rounded-md transition-colors text-sm border border-purple-600/30 w-full justify-start {hasAccess ? 'hover:bg-purple-600/30 cursor-pointer' : 'opacity-50 cursor-not-allowed'}"
            disabled={!hasAccess}
            title={hasAccess ? 'E-Mail-Programm öffnen' : 'Partner Badge erforderlich'}
          >
            ...
            <span class="truncate text-left">{hasAccess ? partner.email : '***@***.com'}</span>
          </button>
        </div>
      {/if}
    </div>
...
<style>
  ...
  /* Blur direkt auf den Inhalt anwenden */
  .locked-partner .blur-content {
    filter: blur(2px);
    -webkit-filter: blur(2px);
  }
  
  /* Hover-Effekt */
  .locked-partner:hover .blur-content {
    filter: blur(3px);
    -webkit-filter: blur(3px);
  }

  /* Overlay für gesperrte Partner */
  .locked-partner::after {
    content: '';
    position: absolute;
    inset: 0;
