...
{#if variant === 'mini'}
  ...
  >
    {#if badge.icon}
      <img 
        src={urlFor(badge.icon).width(64).height(64).url()} 
        alt={badge.name}
        class="w-12 h-12 object-contain {locked ? 'opacity-50' : ''} hover:scale-110 transition-transform"
      />
    {:else}
    ...
    <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
      <div class="bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap border border-gray-700">
        {badge.name}
        {#if locked}
          <span class="text-yellow-400"> (Gesperrt)</span>
        {/if}
      </div>
