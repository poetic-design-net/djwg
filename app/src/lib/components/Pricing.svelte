  ...
  <div class="flex flex-wrap -mx-4">
    {#each pricingPhases as phase}
      <div class="w-full md:w-1/2 lg:w-1/4 px-4 mb-8">
        <div class="relative h-full flex flex-col {phase.status === 'completed' ? 'bg-black/20 border-gray-900' : phase.status === 'current' ? 'bg-black/40 border-purple-500' : 'bg-black/40 border-gray-800'} border rounded-5xl bg-gradient-radial-dark transition duration-200">
          <!-- Content Container -->
          <div class="flex-grow p-8">
            <div class="mb-6">
              <div class="flex items-center justify-between mb-4">
                <span class="text-sm text-tourquis-500">{phase.phase}</span>
              ...
              {#each phase.features as feature}
                <li class="flex items-center mb-4">
                  <svg class="mr-2" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M7.5 10L9.16667 11.6667L12.5 8.33333" stroke={phase.status === 'completed' ? "#4B5563" : phase.status === 'current' ? "#33cc99" : "#4B5563"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M10 17.5C14.1421 17.5 17.5 14.1421 17.5 10C17.5 5.85786 14.1421 2.5 10 2.5C5.85786 2.5 2.5 5.85786 2.5 10C2.5 14.1421 5.85786 17.5 10 17.5Z" stroke={phase.status === 'completed' ? "#4B5563" : phase.status === 'current' ? "#33cc99" : "#4B5563"} stroke-width="1.5"/>
                  </svg>
